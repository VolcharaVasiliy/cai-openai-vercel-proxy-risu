...
- Added stable dialog binding with auto-generated session ids when client does not pass one.
...
- `lib/config.js` - adds deterministic `auto-<hash>` session id generation from first system/user context if no session id is provided.
...
- `resolveSessionId` (`lib/config.js`) - resolves session from headers/user field or deterministic auto-id.
